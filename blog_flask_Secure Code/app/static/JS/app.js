// V-07 FIX: Helper to read the CSRF token from the <meta name="csrf-token"> tag
// injected by base.html. This token is included in all state-changing fetch calls
// so the server can validate it via the X-CSRFToken header (CWE-352 mitigation).
...
// NOTE: Server-side size enforcement has been added (V-12 fix). This JS alert is a UX aid only.
...
function likeOrBookmark(postId, action) {
    ...
    // V-07 FIX: Include X-CSRFToken header in state-changing fetch requests.
    fetch(url, {
        method: "POST",
        headers: new Headers({
...
function commentOrReply(event, postId, isComment) {
    ...
    // V-07 FIX: Include X-CSRFToken header in all JSON POST requests.
    fetch(url, {
        method: "POST",
        credentials: "include",
