...
import bleach  # V-06 FIX: Server-side HTML sanitisation for CKEditor post bodies
...
@login_required
def user_update(id):
    # V-03 FIX: Restrict to admin and super_admin roles only.
...
def submit_post():
    ...
    if form.validate_on_submit():
        ...
        # V-06 FIX: Sanitise CKEditor HTML server-side before storing to prevent stored XSS.
        ALLOWED_TAGS = [
            'p', 'b', 'i', 'u', 'em', 'strong', 'a', 'ul', 'ol', 'li',
            'br', 'h2', 'h3', 'h4', 'blockquote', 'pre', 'code', 'hr', 'img', 'span'
        ]
        ALLOWED_ATTRS = {
            'a': ['href', 'title', 'rel'],
        ...
        body_clean = bleach.clean(
            form.body.data, tags=ALLOWED_TAGS, attributes=ALLOWED_ATTRS, strip=True
        )
        ...
        post = Blog_Posts(theme_id=form.theme.data,
                            ...
                            body=body_clean, picture_v_source=form.picture_v_source.data, picture_h_source=form.picture_h_source.data,
        ...
        # V-12 FIX: Measure file sizes server-side — never trust the client-supplied size field.
        ...
        MAX_IMG_BYTES = 1_500_000  # 1.5 MB

        # checking picture vertical:
        if form.picture_v.data and get_server_file_size(form.picture_v.data) < MAX_IMG_BYTES:
        ...
        if form.picture_h.data and get_server_file_size(form.picture_h.data) < MAX_IMG_BYTES:
        ...
        if form.picture_s.data and get_server_file_size(form.picture_s.data) < MAX_IMG_BYTES:
...
def edit_post(id):
    ...
    # changing the post
    if form.validate_on_submit():
        # V-06 FIX: Sanitise CKEditor HTML before storing (edit path).
        ALLOWED_TAGS = [
            'p', 'b', 'i', 'u', 'em', 'strong', 'a', 'ul', 'ol', 'li',
            'br', 'h2', 'h3', 'h4', 'blockquote', 'pre', 'code', 'hr', 'img', 'span'
        ]
        ...
        # V-12 FIX: Server-side file size enforcement for edit path.
        def get_server_file_size_edit(file_storage):
            file_storage.seek(0, 2)
            size = file_storage.tell()
            file_storage.seek(0)
            return size
        ...
        # checking picture vertical:
        if form.picture_v.data and get_server_file_size_edit(form.picture_v.data) < MAX_IMG_BYTES:
            img_v_filename = secure_filename(form.picture_v.data.filename)
            submit_post_blog_img_handle(img_v_filename, "v")
            submit_post_blog_img_provided["v"] = True
        elif form.picture_v.data and get_server_file_size_edit(form.picture_v.data) >= MAX_IMG_BYTES:
            img_size_not_accepted = True
        else:
            submit_post_blog_img_provided["v"] = bool(post_to_edit.picture_v)

        # checking picture horizontal:
        if form.picture_h.data and get_server_file_size_edit(form.picture_h.data) < MAX_IMG_BYTES:
            img_h_filename = secure_filename(form.picture_h.data.filename)
            submit_post_blog_img_handle(img_h_filename, "h")
            submit_post_blog_img_provided["h"] = True
        elif form.picture_h.data and get_server_file_size_edit(form.picture_h.data) >= MAX_IMG_BYTES:
            img_size_not_accepted = True
        else:
            submit_post_blog_img_provided["h"] = bool(post_to_edit.picture_h)

        # checking picture squared:
        if form.picture_s.data and get_server_file_size_edit(form.picture_s.data) < MAX_IMG_BYTES:
            img_s_filename = secure_filename(form.picture_s.data.filename)
            submit_post_blog_img_handle(img_s_filename, "s")
            submit_post_blog_img_provided["s"] = True
        elif form.picture_s.data and get_server_file_size_edit(form.picture_s.data) >= MAX_IMG_BYTES:
            img_size_not_accepted = True
        else:
            submit_post_blog_img_provided["s"] = bool(post_to_edit.picture_s)
